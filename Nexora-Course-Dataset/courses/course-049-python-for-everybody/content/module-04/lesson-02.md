...
> Official URL: https://www.py4e.com/html3/14-objects
> License: CC BY 4.0
...
The first line _constructs_ an object of type `list`, the second and third lines _call_ the `append()` _method_ , the fourth line calls the `sort()` method, and the fifth line _retrieves_ the item at position 0.
...
In this code, we call the `__getitem__` method in the `list` class and _pass_ the list and the item we want retrieved from the list as parameters.
...
![A Program](../images/program.svg) A Program 
...
![Ignoring Detail When Using an Object](../images/program-oo-code.svg) Ignoring Detail When Using an Object 
...
![Ignoring Detail When Building an Object](../images/program-oo-bs4.svg) Ignoring Detail When Building an Object 
...
An object can contain a number of functions (which we call _methods_) as well as data that is used by those functions. We call data items that are part of the object _attributes_.
...
![A Class and Two Objects](../photos/cookie_cutter_flickr_Didriks.png) A Class and Two Objects 
...
This is where we instruct Python to construct (i.e., create) an _object_ or _instance_ of the class `PartyAnimal`. It looks like a function call to the class itself. Python constructs the object with the right data and methods and returns the object which is then assigned to the variable `an`. In a way this is quite similar to the following line which we have been using all along:
...
This syntax using the _dot_ operator is saying ‘the x within self.’ Each time `party()` is called, the internal `x` value is incremented by 1 and the value is printed out.
...
Another powerful feature of object-oriented programming is the ability to create a new class by extending an existing class. When extending a class, we call the original class the _parent class_ and the new class the _child class_.
...
When we define the `CricketFan` class, we indicate that we are extending the `PartyAnimal` class. This means that all of the variables (`x`) and methods (`party`) from the `PartyAnimal` class are _inherited_ by the `CricketFan` class. For example, within the `six` method in the `CricketFan` class, we call the `party` method from the `PartyAnimal` class.

We use a special syntax in the `__init__()` method in the `CricketFan` class to ensure that we call the `__init()__` method in the `PartyAnimal` so that whatever setup that `PartyAnimal` needs is done in addition to the setup needed for the `CriocketFan` extensions.
...
The first line constructs a `list` _object_. When Python creates the `list` object, it calls the _constructor_ method (named `__init__`) to set up the internal data attributes that will be used to store the list data. We have not passed any parameters to the _constructor_. When the constructor returns, we use the variable `stuff` to point to the returned instance of the `list` class.
...
We then print out the first item in the list using the square brackets which are a shortcut to calling the `__getitem__` method within the `stuff`. This is equivalent to calling the `__getitem__` method in the `list` _class_ and passing the `stuff` object as the first parameter and the position we are looking for as the second parameter.

At the end of the program, the `stuff` object is discarded but not before calling the _destructor_ (named `__del__`) so that the object can clean up any loose ends as necessary.
