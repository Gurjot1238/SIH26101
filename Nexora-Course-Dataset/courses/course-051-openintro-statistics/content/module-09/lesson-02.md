...
Possum classification, Part I 
The common brushtail possum of the Australia region is a bit cuter than its 
distant cousin, the American opossum (see Figure~brushtail_possum). We 
...
regions. The outcome variable, called population, takes value 1 when a 
possum is from Victoria and 0 when it is from New South Wales or Queensland. We 
consider five predictors: sex\_0.3mmmale (an indicator for a 
possum being male), head\_0.3mmlength, skull\_0.3mm
width, total\_0.3mmlength, and tail\_0.3mmlength. 
...
center
ch_regr_mult_and_log/figures/eoce/possum_classification_model_select/possum_variables
center
center
tabularr rrrr r rrrr
                            & 4c**Full Model** &
                            & 4c**Reduced Model**  \\
  2-57-10
-3.1mm \\
...
sex\_0.3mmmale     & -1.2376 & 0.6662  & -1.86 & 0.0632 &
                            & -1.4207 & 0.6457  & -2.20 & 0.0278 \\ 
head\_0.3mmlength  & -0.1601 & 0.1386  & -1.16 & 0.2480 \\ 
skull\_0.3mmwidth  & -0.2012 & 0.1327  & -1.52 & 0.1294 &
                            & -0.2787 & 0.1226  & -2.27 & 0.0231 \\ 
total\_0.3mmlength & 0.6488  & 0.1531  & 4.24  & 0.0000 &
                            & 0.5687  & 0.1322  & 4.30  & 0.0000 \\ 
tail\_0.3mmlength  & -1.8708 & 0.3741  & -5.00 & 0.0000 &
                            & -1.8057 & 0.3599  & -5.02 & 0.0000 \\ 
  
tabular
center
parts
- Examine each of the predictors. Are there any outliers that are likely to 
have a very large influence on the logistic regression model?
- The summary table for the full model indicates that at least one variable 
should be eliminated when using the p-value approach for variable selection: 
head\_0.3mmlength. The second component of the table summarizes 
...
is based on the temperature at the time of the launch. **Temp** gives the 
temperature in Fahrenheit, **Damaged** represents the number of damaged O-
rings, and **Undamaged** represents the number of O-rings that were not 
damaged.
center
tabularl rrrrr rrrrr rrrrr rrrrr rrr
...
- Failures have been coded as 1 for a damaged O-ring and 0 for an undamaged 
...
parameters.
- Based on the model, do you think concerns regarding O-rings are justified? 
Explain.
parts

Possum classification, Part II 
A logistic regression model was proposed for classifying common brushtail 
possums into their two regions in 
Exercise~. The outcome variable took 
...
sex\_0.3mmmale     & -1.4207   & 0.6457  & -2.20 & 0.0278 \\ 
skull\_0.3mmwidth  & -0.2787   & 0.1226  & -2.27 & 0.0231 \\ 
total\_0.3mmlength & 0.5687    & 0.1322  & 4.30  & 0.0000 \\ 
tail\_0.3mmlength  & -1.8057   & 0.3599  & -5.02 & 0.0000 \\ 
  
tabular
center
parts
- Write out the form of the model. Also identify which of the variables are 
positively associated when controlling for other variables.
- Suppose we see a brushtail possum at a zoo in the US, and a sign says the 
...
Challenger disaster, Part II 
Exercise~ introduced us to O-rings that 
...
center
0.6ch_regr_mult_and_log/figures/eoce/challenger_disaster_predict/challenger_disaster_damage_temp.pdf 
center
parts
- The data provided in the previous exercise are shown in the plot. The logistic 
model fit to these data may be written as
align*
( p1 - p ) = 11.6630 - 0.2162 Temperature
align*
where $p$ is the model-estimated probability that an O-ring will become 
...
&p_65 = 0.084
	...
	&& p_71 = 0.024
align*
- Add the model-estimated probabilities from part~(a) on the plot, then 
connect these dots using a smooth curve to represent the model-estimated 
probabilities.
- Describe any concerns you may have regarding applying logistic regression 
